Attribute VB_Name = "alphabetical_testing"
...
'To ensure that last row value is added when ticker changes
For I = 2 To LR
    ...
    Else
        'total value added when ticker value changes
        tickervolume = tickervolume + Cells(I, 7).Value
    
    End If
        
    Next I
        'to find max and min ticker value and max total value
